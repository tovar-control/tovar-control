  ...
  //   selects.append('<option value="KG" ' + kg_selected + '>Кыргызстан</option>');
    ...
    var change = 0,
        updatePrices = function (item) {
            ...
            if (sel == 'RU') {
                $('.old_price_val').html('2275');
                $('.old_price_cur').html('руб*');
                $('.old_price_sig').html('&#8381;');
                $('.new_price_val').html('99');
                $('.new_price_cur').html('руб*');
                $('.new_price_sig').html('&#8381;');
                $('.img_price').attr('src', 'src/product-RU.png');
            ...
            if (sel == 'UA') {
                ...
                $('.img_price').attr('src', 'src/product-UA.png');
            ...
            if (sel == 'GE') {
                $('.old_price_val').html('100');
                $('.old_price_cur').html('gel*');
                $('.old_price_sig').html('gel*');
                $('.new_price_val').html('10');
                $('.new_price_cur').html('gel*');
                $('.new_price_sig').html('gel*');
                $('select').val(sel).trigger('change');
                initializeMask({mask:"(+\\9\\95)99999999[99]",removeMaskOnSubmit: false})
            }

            if (sel == 'BY') {
                $('.old_price_val').html('1');
            ...
            if (sel == 'KG') {
                ...
                $('select').val(sel).trigger('change');
				initializeMask({mask:"(+\\9\\96)999999999[9]",removeMaskOnSubmit: false})
            }
